window.mysecskilldetail=(function() {
			...
			getajax.getAction(server, params, function(res) {
				...
				if (res['code'] == 200) {
					if (!res['datas']['error']) {
						...
						setTimeout(function() {
						//window.location.href = '../order/confirmorder.html?id=' + buyid;
			...
			getajax.getAction(server, params, function(res) {
				...
				if (res['code'] == 200) {
					...
					//倒计时
							var that=$('.time');
							//console.log(that)
							var strong=$('.time>strong');
							var s1=$('.time>span:nth-of-type(1)');
							var s2=$('.time>span:nth-of-type(2)');
							var s3=$('.time>span:nth-of-type(3)');
							if(goods_storage==0){
								strong.text('活动已结束：');
								s1.text('00');
								s2.text('00');
								...
								var _endtime=parseInt(data['end_time']);
								//_endtime=1442689854;
								var _strtime=parseInt(data['start_time']);
								//_strtime=1442562854;
								var t1=_strtime-myTime;
								var t2=_endtime-myTime;
								var _hours=0;
								var _min=0;
								var _sec=0;
								var str='';
								...
								function cuttime(t){
										_hours=parseInt(t/3600);
										_min=parseInt(t%3600/60);
									 	_sec=parseInt(t%3600%60);
